 ...
 *
 */
function bp_care_is_liked( $item_id = '' , $type = '' , $user_id = '' ) {
    global $bp;
    ...
    if ( $type == 'activity' ) {
        $user_likes = get_user_meta( $user_id , 'bp_cared_activities' , true );
    }

    if ( $type == 'blogpost' ) {
        $user_likes = get_user_meta( $user_id , 'bp_cared_blogposts' , true );
    }
 ...
 *
 */
function bp_care_add_user_like( $item_id = '' , $type = 'activity' ) {
    global $bp;
    ...
    if ( $type == 'activity' ) {
        ...
        update_user_meta( $user_id , 'bp_cared_activities' , $user_likes );
        ...
        bp_care_post_to_stream( $item_id , $user_id );
    } elseif ( $type == 'blogpost' ) {

        /* Add to the users liked blog posts. */
        $user_likes = get_user_meta( $user_id , 'bp_cared_blogposts' , true );
        $user_likes[$item_id] = 'blogpost_liked';
        update_user_meta( $user_id , 'bp_cared_blogposts' , $user_likes );
        ...
        if ( bp_care_get_settings( 'post_to_activity_stream' ) == 1 ) {
            ...
            if ( $user_id == $author_id ) {
                $action = bp_care_get_text( 'record_activity_likes_own_blogpost' );
            } elseif ( $user_id == 0 ) {
                $action = bp_care_get_text( 'record_activity_likes_a_blogpost' );
            } else {
                $action = bp_care_get_text( 'record_activity_likes_users_blogpost' );
            }
            ...
            if ( bp_care_get_settings( 'show_excerpt' ) == 1 ) {
                $content = $post->post_content;
                if ( strlen( $content ) > bp_care_get_settings( 'excerpt_length' ) ) {
                    $content = substr( $content , 0 , bp_care_get_settings( 'excerpt_length' ) );
                    $content = $content . '...';
                }
            };
...
/**
 * bp_care_remove_user_like()
 *
 * Registers that the user has unliked a given item.
 *
 */
function bp_care_remove_user_like( $item_id = '' , $type = 'activity' ) {
    global $bp;
    ...
    if ( $user_id == 0 ) {
        echo bp_care_get_text( 'must_be_logged_in' );
        return false;
    }
 ...
 *
 */
function bp_care_get_likes( $item_id = '' , $type = '' , $user_id = '' ) {
    global $bp;
    ...
    if ( $type == 'activity' ) {
        ...
        if ( $liked_count == 0 ) {
            $output .= bp_care_get_text( 'get_likes_no_likes' );
        }

        /* We should show information about all likers */ elseif ( bp_care_get_settings( 'likers_visibility' ) == 'show_all' ) {

            /* Settings say we should show their name. */
            if ( bp_care_get_settings( 'name_or_avatar' ) == 'name' ) {

                /* Current user likes it too, remove them from the liked count and output appropriate message */
                if ( bp_care_is_liked( $item_id , 'activity' , $user_id ) ) {
                    ...
                    if ( $liked_count == 1 ) {
                        ...
                        $output .= bp_care_get_text( 'get_likes_you_and_singular' );
                        ...
                        $output .= bp_care_get_text( 'get_likes_only_liker' );
                    } else {
                        $output .= bp_care_get_text( 'get_likes_you_and_plural' );
                    }
                } else {

                    if ( $liked_count == 1 ) {
                        $output .= bp_care_get_text( 'get_likes_count_people_singular' );
                    } else {
                        $output .= bp_care_get_text( 'get_likes_count_people_plural' );
                    }
                }
            ...
            /* Settings say we should show their avatar. */ elseif ( bp_care_get_settings( 'name_or_avatar' ) == 'avatar' ) {
        ...
        /* We should show the information of friends, but only the number of non-friends. */ elseif ( bp_care_get_settings( 'likers_visibility' ) == 'friends_names_others_numbers' && bp_is_active( 'friends' ) ) {
            ...
            if ( bp_care_get_settings( 'name_or_avatar' ) == 'name' ) {
                ...
                if ( $liked_count == 1 ) {
                    $output .= bp_care_get_text( 'get_likes_and_people_singular' );
                } elseif ( $liked_count > 1 ) {
                    $output .= bp_care_get_text( 'get_likes_and_people_plural' );
                } else {
                    $output .= bp_care_get_text( 'get_likes_like_this' );
                }
            }
            /* Settings say we should show their avatar. */ elseif ( bp_care_get_settings( 'name_or_avatar' ) == 'avatar' ) {
            ...
            } elseif ( bp_care_get_settings( 'likers_visibility' ) == 'friends_names_others_numbers' && !bp_is_active( 'friends' ) || bp_care_get_settings( 'likers_visibility' ) == 'just_numbers' ) {
...
/**
 * 
 * bp_care_users_who_like()
 * 
 * Gets the usernames of users who liked post id
 * 
 */
function bp_care_users_who_like( $bp_care_id = '' ) {
    ...
    if ( count( $users_who_like ) == 1 ) {
        echo bp_core_get_userlink( $users_who_like[0] , $no_anchor = false , $just_link = false ) . _e( 'liked this' , 'bp-care' );
    } else {
        for ( $i = count( $users_who_like ) - 1; $i >= 0; $i-- ) {
            echo bp_core_get_userlink( $users_who_like[0] , $no_anchor = false , $just_link = false )
            . _e( 'and' , 'bp-care' ) . count( $users_who_like ) - 1
            . _e( 'liked this' , 'bp-care' );
        }
    }
}

/*
 * bp_care_get_some_likes()
 * 
 ...
 */
function bp_care_get_some_likes( $number = '' ) {
    ...
    $bp_care_id = bp_get_activity_id();
    $users_who_like = array_keys((array)(bp_activity_get_meta( $bp_care_id , 'liked_count' , true )));
...
add_action( 'view_who_likes' , 'bp_care_get_some_likes', 1000 );
